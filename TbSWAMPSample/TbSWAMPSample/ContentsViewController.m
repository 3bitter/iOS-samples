...
@interface ContentsViewController ()<TbBTManagerDelegate>
...
- (void)viewDidLoad {
    ...
    // このサンプルでは準備に失敗した場合はビーコン処理を全て無視します
    AppDelegate *delegate = (AppDelegate *)[UIApplication sharedApplication].delegate;
    if (delegate.skipSWAMPExec) {
        _swampUsed = NO;
    } else {
        _swampUsed = YES;
    }
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (_ignoreUserSettings && _swampUsed) {
...
- (void)checkBluetoothState {
    ...
    assert(_btManager != nil);
    _btManager.delegate = self;
    [_btManager checkCurrentBluetoothAvailability];
}
